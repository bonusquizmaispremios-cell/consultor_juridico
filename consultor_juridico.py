...
if st.session_state.etapa == "Login":
    ...
    with _tab_Home:
        ...
        exemplos_placeholder = {
            "Consumidor": "ex: Comprei um produto online e quero devolver, mas a loja não aceita...",
            "Trabalho": "ex: Fui demitido sem aviso prévio e não recebi as horas extras que fiz...",
            "Moradia": "ex: O proprietário quer me despejar antes do fim do contrato de aluguel...",
            "Contratos": "ex: Assinei um contrato de prestação de serviço e quero cancelar, tem multa?",
            "Transito": "ex: Recebi uma multa que acho injusta, posso recorrer? Como funciona?",
            "Familia": "ex: Quero saber como funciona a pensão alimentícia, quanto o pai deve pagar...",
            "Condominio": "ex: O condomínio quer me multar por causa do meu cachorro, isso é legal?",
            "MEI": "ex: Quero abrir um MEI mas não sei quais são as obrigações mensais...",
        }

        col1, col2 = st.columns(2)
        with col1:
            pergunta = st.text_area("📝 Descreva sua situação ou pergunta:", height=140,
                placeholder=exemplos_placeholder.get(st.session_state.pagina, "Descreva sua situação..."),
                key=f"pergunta_{st.session_state.pagina}")
        with col2:
            st.markdown("**💡 Perguntas comuns nesta área:**")
            perguntas_comuns = {
                "Consumidor": ["Posso devolver um produto sem defeito?", "Quanto tempo tenho de garantia legal?", "A loja pode recusar troca?"],
                "Trabalho": ["Tenho direito a horas extras?", "Como funciona o aviso prévio?", "Posso ser demitido de férias?"],
                "Moradia": ["Posso sair do aluguel antes do prazo?", "O que o fiador é responsável por pagar?", "Posso reter o depósito caução?"],
                "Contratos": ["Posso cancelar um contrato assinado?", "Cláusula de multa é sempre válida?", "O que é cláusula abusiva?"],
                "Transito": ["Posso recorrer de uma multa?", "Quem paga em caso de batida?", "Perdi pontos, e agora?"],
                "Familia": ["Como é calculada a pensão alimentícia?", "Como funciona a guarda compartilhada?", "União estável tem os mesmos direitos do casamento?"],
                "Condominio": ["O síndico pode me multar sem aviso?", "Posso ter animal no apartamento?", "Posso me recusar a pagar taxa extra?"],
                "MEI": ["Quais impostos o MEI paga?", "Posso ter funcionário como MEI?", "O que acontece se eu não pagar o DAS?"],
            }
            for p in perguntas_comuns.get(st.session_state.pagina, []):
                if st.button(f"→ {p}", key=f"sug_{st.session_state.pagina}_{p[:20]}"):
                    st.session_state[f'pergunta_sugerida_{st.session_state.pagina}'] = p

        if st.session_state.get(f'pergunta_sugerida_{st.session_state.pagina}'):
            pergunta = st.session_state[f'pergunta_sugerida_{st.session_state.pagina}']
            st.info(f"📋 Pergunta selecionada: *{pergunta}*")

        if st.button(f"⚖️ OBTER ORIENTAÇÃO", key=f"btn_consultar_{st.session_state.pagina}"):
            if pergunta.strip():
                with st.spinner("Analisando sua situação..."):
                    prompt = (
                        f"Pergunta/situação do usuário sobre {m['titulo']}: {pergunta}\n\n"
                        f"FORMATO DE RESPOSTA:\n\n"
                        f"📋 RESUMO DA SUA SITUAÇÃO:\n[1-2 linhas confirmando o entendimento]\n\n"
                        f"⚖️ O QUE A LEGISLAÇÃO PREVÊ EM GERAL:\n[Explicação clara dos direitos/regras aplicáveis, citando a lei/norma quando relevante]\n\n"
                        f"✅ O QUE VOCÊ PODE FAZER:\n[Passos práticos e concretos]\n\n"
                        f"📂 DOCUMENTOS/PROVAS QUE PODEM AJUDAR:\n[Liste o que reunir para esse caso específico]\n\n"
                        f"⚠️ PONTOS DE ATENÇÃO:\n[Prazos, exceções ou cuidados importantes para esse tipo de situação]\n\n"
                        f"🏛️ SE PRECISAR DE MAIS AJUDA:\n[Para esse tipo de caso, indique o órgão/canal mais adequado — Procon, Defensoria, sindicato, etc]"
                    )
                    res = juridico_ia(prompt, m['titulo'])
                    if res: st.session_state['res_home_consul1'] = str(res)
                    salvar_consulta(st.session_state.pagina, pergunta, res)
                    st.session_state[f'resposta_{st.session_state.pagina}'] = res
            else:
                st.warning("Descreva sua situação ou pergunta.")

        chave_resp = f'resposta_{st.session_state.pagina}'
        if st.session_state.get(chave_resp):
            st.markdown(f"<div class='{m['cor']}'>{st.session_state[chave_resp]}</div>", unsafe_allow_html=True)
            st.markdown(DISCLAIMER_PADRAO, unsafe_allow_html=True)

            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar orientação (.txt)", data=st.session_state[chave_resp],
                    file_name=f"orientacao_{st.session_state.pagina.lower()}.txt", mime="text/plain",
                    use_container_width=True, key=f"dl_{st.session_state.pagina}")
            with col_sv:
                if st.button("❤️ Salvar consulta", key=f"sv_{st.session_state.pagina}", use_container_width=True):
                    st.session_state.consultas_salvas.append({
                        'modulo': st.session_state.pagina, 'pergunta': pergunta,
                        'conteudo': st.session_state[chave_resp],
                        'data': datetime.now().strftime('%d/%m %H:%M'),
                    })
                    st.success("❤️ Salvo!")
        ...
        with _csl1:
            ...
            st.download_button("💾 Salvar dados (.json)",
                ...
                file_name=f"dados_{st.session_state.get('usuario','user')}.json",
                mime="application/json", key="dl_sv_consulto")
        with _csl2:
            _arq_sv = st.file_uploader("📂 Carregar dados:", type=["json"], key="ul_sv_consulto")
            if _arq_sv:
                try:
